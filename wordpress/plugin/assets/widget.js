...
(function () {
  ...
  var SITE_ENDPOINT = cfg.siteEndpoint || "";
  ...
  // Fetch the active site profile so the title/greeting follow the backend's
  // DEMO_SITE_PROFILE. Resolves regardless of success so the UI always builds;
  // on failure the configured defaults are kept.
  function applySiteProfile() {
    if (!SITE_ENDPOINT) return Promise.resolve();
    var headers = {};
    if (MODE === "proxy" && NONCE) headers["X-WP-Nonce"] = NONCE;
    return fetch(SITE_ENDPOINT, { headers: headers })
